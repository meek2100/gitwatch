...
@test "quiet_mode_suppresses_output_q_flag_suppresses_verbose_and_standard_output" {
  local output_file
  ...
  # shellcheck disable=SC2154 # testdir is sourced via setup function
  "${BATS_TEST_DIRNAME}/../gitwatch.sh" -q "${GITWATCH_TEST_ARGS_ARRAY[@]}" "$testdir/local/$TEST_SUBDIR_NAME" > "$output_file" 2>&1 &
...
@test "quiet_mode_suppresses_warnings_q_flag_suppresses_startup_warnings" {
  local output_file
  # shellcheck disable=SC2154 # testdir is sourced via setup function
  output_file=$(mktemp "$testdir/output.XXXXX")
  cd "$testdir/local/$TEST_SUBDIR_NAME"
...
@test "quiet_mode_suppresses_errors_q_flag_suppresses_critical_error_output" {
  local output_file
  # shellcheck disable=SC2154 # testdir is sourced via setup function
  output_file=$(mktemp "$testdir/output.XXXXX")
