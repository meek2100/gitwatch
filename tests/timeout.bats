...
# Force timeout to 3s for faster tests, ignoring any global args
TEST_TIMEOUT=3
...
@test "timeout_git_push_ensures_hung_git_push_command_is_terminated_and_logged" {
  ...
  "$stdbuf_cmd" -oL -eL "${BATS_TEST_DIRNAME}/../gitwatch.sh" "${GITWATCH_TEST_ARGS_ARRAY[@]}" -t "$TEST_TIMEOUT" -s "$test_sleep_time" -r origin "$target_dir" &> "$output_file" &
...
@test "timeout_git_pull_rebase_ensures_hung_git_pull_command_is_terminated_and_logged" {
  ...
  verbose_echo "# DEBUG: Using stdbuf command: '$stdbuf_cmd'"
  # --- MODIFIED: Changed redirection to &> ---
  "$stdbuf_cmd" -oL -eL "${BATS_TEST_DIRNAME}/../gitwatch.sh" "${GITWATCH_TEST_ARGS_ARRAY[@]}" -t "$TEST_TIMEOUT" -s "$test_sleep_time" -r origin -R "$target_dir" &> "$output_file" &
  # shellcheck disable=SC2034 # used by teardown
  GITWATCH_PID=$!
  cd "$target_dir"
  sleep 1 # Allow watcher to initialize
...
@test "timeout_git_commit_ensures_hung_git_commit_command_is_terminated_and_logged" {
  ...
  verbose_echo "# DEBUG: Using stdbuf command: '$stdbuf_cmd'"
  # --- MODIFIED: Changed redirection to &> ---
  "$stdbuf_cmd" -oL -eL "${BATS_TEST_DIRNAME}/../gitwatch.sh" "${GITWATCH_TEST_ARGS_ARRAY[@]}" -t "$TEST_TIMEOUT" -s "$test_sleep_time" "$target_dir" &> "$output_file" &
  # shellcheck disable=SC2034 # used by teardown
  GITWATCH_PID=$!
  sleep 1 # Allow watcher to initialize
