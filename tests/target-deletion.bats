...
@test "target_deletion_file: Commits deletion and exits gracefully" {
  ...
  "${BATS_TEST_DIRNAME}/../gitwatch.sh" "${GITWATCH_TEST_ARGS_ARRAY[@]}" -l 10 "$watched_file_path" > "$output_file" 2>&1 &
  ...
  if kill -0 "$watch_pid" 2>/dev/null;
  then
...
@test "target_deletion_dir: Exits gracefully when the watched directory is deleted" {
  ...
  "${BATS_TEST_DIRNAME}/../gitwatch.sh" "${GITWATCH_TEST_ARGS_ARRAY[@]}" "$sub_dir_path" > "$output_file" 2>&1 &
  # shellcheck disable=SC2034 # not needed locally, PID used below
  GITWATCH_PID=$!
  local watch_pid=$GITWATCH_PID
  sleep 1 # Allow watcher to initialize
